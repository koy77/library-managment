...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h3">Редактировать книгу</h1>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('author_id') is-invalid @enderror" id="author_id" name="author_id" required>
                            @foreach ($authors as $author)
                                <option value="{{ $author->id }}" @selected(old('author_id', $book->author_id) == $author->id)>{{ $author->full_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" class="form-control @error('total_copies') is-invalid @enderror"
                               id="total_copies" name="total_copies" value="{{ old('total_copies', $book->total_copies) }}" min="0" required>
                        <div class="form-text">Выдано на руки: {{ $book->active_issues_count }}</div>
